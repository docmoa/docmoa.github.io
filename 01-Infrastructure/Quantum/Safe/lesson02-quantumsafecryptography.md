---
url: /01-Infrastructure/Quantum/Safe/lesson02-quantumsafecryptography.md
...
| 보안 속성 | Quantum-safe Cryptography (QSC)양자 안전 암호화 | Quantum Key Distribution (QKD)양자 키 분배 |
|---------|------------------------------------------------|--------------------------------------------|
| **설명** | 고전 및 양자 컴퓨터 모두에 대해 해결하기 어려운 수학적 문제에 기반한 암호화 | 양자 물리 현상을 활용하여 두 원격 당사자 간에 암호화 키를 교환할 수 있게 함 |
| **Confidentiality(기밀성)** | ✓ 지원 | ✓ 지원 |
| **Authentication(인증)** | ✓ 지원 | ✗ 지원하지 않음 |
| **Integrity(무결성)** | ✓ 지원 | ✗ 지원하지 않음 |
| **Non-Repudiation(부인 방지)** | ✓ 지원 | ✗ 지원하지 않음 |
