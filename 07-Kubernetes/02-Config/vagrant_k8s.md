---
url: /07-Kubernetes/02-Config/vagrant_k8s.md
...
|Variable name|value|
|-|-|
|IMAGE\_NAME|vagrant에서 사용할 기본 이미지로 vagrant cloud 참조|
|K8S\_MINOR\_VERSION|K8s 설치의 마이너 버전 지정|
|NETWORK\_SUB|Virtualbox network ip|
|START\_IP|각 K8s 클러스터의 master에 할당되며 워커노드는 +1 씩 증가|
|POD\_CIDR|kubeadm init 의 --pod-network-cidr 에 지정되는 CIDR|
|cluster={}|클러스터 리소스를 정의한 오브젝트 형태의 변수|
|NODE\_COUNT|워커 노드 개수|
|VM\_GROUP\_NAME|Virtualbox에 등록될 그룹 이름|
|DOCKER\_VER|docker-ce 버전|
|KUBE\_VER|K8s 관련 패키지 버전|
