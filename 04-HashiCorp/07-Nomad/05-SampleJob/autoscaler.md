---
url: /04-HashiCorp/07-Nomad/05-SampleJob/autoscaler.md
...
* Nomad Autoscaler sampe job의 `target "aws-asg"` 설정방법
  * aws\_asg\_name은 aws cloud 환경의 Auto Scaling 그룹에 존재해야 함.
  * node\_class는 nomad client에 동일하게 설정해야 함.
* 주요 튜닝 포인트
  * `policy`의   cooldown,  evaluation\_interval 값을 워크로드 특성에 맞게 적절하게 설정
* 오토스케일링 기준
