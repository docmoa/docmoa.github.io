---
url: /04-HashiCorp/07-Nomad/05-SampleJob/jenkins_java_driver.md
...
* resources
  * memory\_max : 유동적 메모리 할당
  * network : Jenkins에서 사용할 포트 지정
* env : Jenkins Home 디렉토리 위한 환경 변수 `JENKINS_HOME` 선언
* config
  * jar\_path : `artifact`로 받은 war파일 경로
  * jvm\_options : jvm 실행 옵션
  * args : war 실행 문서에서 지정하는 `--httpPort`는 args 항목이고 jvm 옵션이 아님에 주의
* volume\_mount : Jenkins Home 디렉토리로 Host Volume으로 지정한 영구적 볼륨 할당
