---
url: /04-HashiCorp/06-Vault/03-Auth_Method/aws-auth.md
...
ec2 방식에서는 AWS가 신뢰할 수 있는 제3자로 취급되며, 각 EC2 인스턴스를 고유하게 나타내는 암호화 서명된 동적 메타데이터 정보가 인증에 사용된다. 이 메타데이터 정보는 AWS가 모든 EC2 인스턴스에 자동으로 제공한다. 특정 AMI, 특정 인스턴스 프로파일의 EC2 인스턴스 또는 특수 태그 값을 가진 EC2 인스턴스(role\_tag 기능을 통해)에서 EC2 인스턴스에 대한 액세스를 제한하는 등 EC2 인스턴스를 처리하는 데 특화되어 있다.
...
* iam\_request\_url 값 예시 : `https://sts.amazonaws.com/`

* iam\_request\_body 값 예시 : `Action=GetCallerIdentity&Version=2011-06-15`

* iam\_request\_headers 값 예시 :
