...
Root Token: root
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><code>root</code> 토큰은 구성관리 관리자의 권한으로 가정한다.</p><div class="language-bash line-numbers-mode" data-ext="sh" data-title="sh"><pre class="language-bash"><code>$ <span class="token builtin class-name">export</span> <span class="token assign-left variable">VAULT_ADDR</span><span class="token operator">=</span><span class="token string">&#39;http://127.0.0.1:8200&#39;</span>
$ vault login
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>Spring Boot 앱에서 사용할 KV를 활성화 한다.</p><div class="language-bash" data-ext="sh" data-title="sh"><pre class="language-bash"><code>$ vault secrets <span class="token builtin class-name">enable</span> <span class="token parameter variable">-path</span><span class="token operator">=</span>demo-app <span class="token parameter variable">-version</span><span class="token operator">=</span><span class="token number">2</span> kv

Success<span class="token operator">!</span> Enabled the kv secrets engine at: demo-app/
</code></pre></div>`,16),k={href:"https://start.spring.io",target:"_blank",rel:"noopener noreferrer"},v=t(`<figure><img src="https://raw.githubusercontent.com/Great-Stone/images/master/uPic/image-20230406150032627.png" alt="image-20230406150032627" tabindex="0" loading="lazy"><figcaption>image-20230406150032627</figcaption></figure><p>테스트를 위한 종속성 목록은 다음과 같다.</p><table><thead><tr><th>Dependencies</th><th>설명</th></tr></thead><tbody><tr><td>Spring Web</td><td>Spring MVC를 사용하여 RESTful을 포함한 웹 애플리케이션 구축에 사용</td></tr><tr><td>MySQL Driver</td><td>MySQL을 사용하기위한 드라이버 (MySQL 없는 경우 생략)</td></tr><tr><td>Spring Data JPA</td><td>JPA를 사용하기 편하도록 만들어놓은 모듈 (MySQL 없는 경우 생략)</td></tr><tr><td>Vault Configuration</td><td>분산 시스템에서 외부화된 볼트 구성에 대한 클라이언트 측 지원을 제공</td></tr><tr><td>Lombok</td><td>기계적인 코드들을 어노테이션을 기반으로 코드를 자동화하여 작성해주는 Java의 라이브러리</td></tr></tbody></table><p>MySQL의 경우 다음과 같이 구성한다.</p><div class="language-sql" data-ext="sql" data-title="sql"><pre class="language-sql"><code><span class="token keyword">CREATE</span> <span class="token keyword">DATABASE</span> java_dev_db<span class="token punctuation">;</span>
...
</code></pre></div><p>앱에서 사용할 구성을 볼트의 <code>demo-app/java_and_vault/dev</code>에 추가한다. 엔드포인트 정보의 조합은 <code>&lt;kv_endpoint&gt;/&lt;app_name&gt;/&lt;profile&gt;</code> 이다. 다음과 같이 CLI를 사용하여 구성 정보를 추가한다.</p><div class="language-bash" data-ext="sh" data-title="sh"><pre class="language-bash"><code>$ vault kv put demo-app/java_and_vault/dev <span class="token punctuation">\\</span>
...
</code></pre></div><p>UI에서 확인해보면 결과는 다음과 같다.</p><figure><img src="https://raw.githubusercontent.com/Great-Stone/images/master/uPic/image-20230407093720360.png" alt="image-20230407093720360" tabindex="0" loading="lazy"><figcaption>image-20230407093720360</figcaption></figure><p>앱과 볼트 연동 구성을 위해 다음을 추가한다. 기존 <code>application.properties</code> 대신 <code>application.yml</code>로 변경하여 구성한다.</p><div class="language-yaml line-numbers-mode" data-ext="yml" data-title="yml"><pre class="language-yaml"><code><span class="token key atrule">spring</span><span class="token punctuation">:</span>
...
</code></pre></div><p><code>prd</code>를 위한 구성정보를 볼트에 추가한다.</p><div class="language-bash" data-ext="sh" data-title="sh"><pre class="language-bash"><code>$ vault kv put demo-app/java_and_vault/prd <span class="token punctuation">\\</span>
...
</code></pre></div><p>구성을 읽을수만 있는 Policy <code>java-and-vault-prd-read.hcl</code>파일 내용 및 적용은 다음과 같다.</p><div class="language-bash" data-ext="sh" data-title="sh"><pre class="language-bash"><code>$ <span class="token function">cat</span> java-and-vault-prd-read.hcl
...
</code></pre></div><p>앱을 위한 계정을 발급하기위한 Policy인 <code>java-and-vault-prd-approle.hcl</code> 파일 내용은 다음과 같다.</p><div class="language-bash line-numbers-mode" data-ext="sh" data-title="sh"><pre class="language-bash"><code>$ <span class="token function">cat</span> java-and-vault-prd-approle.hcl
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>관리자에게 <code>java-and-vault-prd-admin</code>, <code>java-and-vault-prd-approle</code> 를 부여하여 구성에 대한 관리와 앱을위한 계정 발급 권한을 준다.</p><div class="language-bash" data-ext="sh" data-title="sh"><pre class="language-bash"><code><span class="token comment"># 활성화 되어있지 않다면 userpass Auth Method 활성화</span>
...
</code></pre></div><p>앱을 위한 AppRole인증에 <code>java-and-vault-prd-read</code>를 추가한다.</p><div class="language-bash line-numbers-mode" data-ext="sh" data-title="sh"><pre class="language-bash"><code><span class="token comment"># 활성화 되어있지 않다면 approle Auth Method 활성화</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>생성한 관리자 계정으로 로그인 하면 <code>demo-app/java_and_vault/prd</code> 의 구성 변경과 AppRole 계정의 <code>secret-id</code> 발급이 가능한지 확인한다. (별도의 터미널)</p><div class="language-bash line-numbers-mode" data-ext="sh" data-title="sh"><pre class="language-bash"><code>$ <span class="token builtin class-name">export</span> <span class="token assign-left variable">VAULT_ADDR</span><span class="token operator">=</span>http://127.0.0.1:8200
...
secret_id_ttl         10m
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>앱에서 사용할 AppRole 계정으로 로그인 하면 <code>demo-app/java_and_vault/prd</code> 의 구성 변경을 읽을수는 있고 업데이트는 안되는 여부를 확인한다. (별도의 터미널)</p><div class="language-bash line-numbers-mode" data-ext="sh" data-title="sh"><pre class="language-bash"><code>$ <span class="token builtin class-name">export</span> <span class="token assign-left variable">VAULT_ADDR</span><span class="token operator">=</span>http://127.0.0.1:8200
...
* <span class="token number">1</span> error occurred:
	* permission denied
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>앱과 정책이 적용된 볼트 연동 구성을 위해 <code>application.yml</code>를 수정한다.</p><div class="language-yaml line-numbers-mode" data-ext="yml" data-title="yml"><pre class="language-yaml"><code><span class="token key atrule">spring</span><span class="token punctuation">:</span>
  <span class="token key atrule">application</span><span class="token punctuation">:</span>
    <span class="token key atrule">name</span><span class="token punctuation">:</span> java_and_vault
  <span class="token key atrule">cloud.vault</span><span class="token punctuation">:</span>
      <span class="token key atrule">host</span><span class="token punctuation">:</span> 127.0.0.1
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>권한이 없는 <code>dev</code> 프로파일을 지정하는 경우 구성 값을 가져오지 못하므로 앱이 실행될 때 에러가 발생한다.</p>`,51);function m(b,g){const s=o("ExternalLinkIcon");return l(),i("div",null,[r,n("blockquote",null,[n("p",null,[a("Example Source : "),n("a",u,[a("https://github.com/Great-Stone/vault_springboot_example"),e(s)])])]),d,n("p",null,[a("예제에서는 구성관리에서 MySQL 정보를 관리한다고 가정합니다. 관련 Spring Boot 앱은 "),n("a",k,[a("spring initializr"),e(s)]),a("를 통해 생성한다.")]),v])}const y=p(c,[["render",m],["__file","spring-boot.html.vue"]]),f=JSON.parse('{"path":"/04-HashiCorp/06-Vault/04-UseCase/spring-boot.html","title":"Vault로 Spring Boot 구성관리","lang":"ko-KR","frontmatter":{"head":[["meta",{"name":"Spring Boot Vault","content":"Spring Boot에 구성관리로 Vault 사용"}],["meta",{"property":"og:url","content":"https://docmoa.github.io/04-HashiCorp/06-Vault/04-UseCase/spring-boot.html"}],["meta",{"property":"og:site_name","content":"docmoa"}],["meta",{"property":"og:title","content":"Vault로 Spring Boot 구성관리"}],["meta",{"property":"og:description","content":"Vault로 Spring Boot 구성관리 Example Source : https://github.com/Great-Stone/vault_springboot_example 볼트는 애플리케이션(앱)의 구성관리, 특히 사용자 ID, 패스워드, Token, 인증서, 엔드포인트, AWS 자격증명 등과 같은 민감한 정보를 ..."}],["meta",{"property":"og:type","content":"article"}],["meta",{"property":"og:image","content":"https://raw.githubusercontent.com/Great-Stone/images/master/uPic/image-20230406150032627.png"}],["meta",{"property":"og:locale","content":"ko-KR"}],["meta",{"property":"og:updated_time","content":"2023-09-18T13:12:54.000Z"}],["meta",{"property":"article:tag","content":"vault"}],["meta",{"property":"article:tag","content":"java"}],["meta",{"property":"article:tag","content":"spring"}],["meta",{"property":"article:modified_time","content":"2023-09-18T13:12:54.000Z"}],["script",{"type":"application/ld+json"},"{\\"@context\\":\\"https://schema.org\\",\\"@type\\":\\"Article\\",\\"headline\\":\\"Vault로 Spring Boot 구성관리\\",\\"image\\":[\\"https://raw.githubusercontent.com/Great-Stone/images/master/uPic/image-20230406150032627.png\\",\\"https://raw.githubusercontent.com/Great-Stone/images/master/uPic/image-20230407093720360.png\\",\\"https://raw.githubusercontent.com/Great-Stone/images/master/uPic/image-20230407094227005.png\\"],\\"dateModified\\":\\"2023-09-18T13:12:54.000Z\\",\\"author\\":[]}"]],"tag":["vault","java","spring"],"description":"Vault로 Spring Boot 구성관리 Example Source : https://github.com/Great-Stone/vault_springboot_example 볼트는 애플리케이션(앱)의 구성관리, 특히 사용자 ID, 패스워드, Token, 인증서, 엔드포인트, AWS 자격증명 등과 같은 민감한 정보를 ..."},"headers":[{"level":2,"title":"구성관리 개념","slug":"구성관리-개념","link":"#구성관리-개념","children":[]},{"level":2,"title":"볼트의 구성관리 저장소 및 관리","slug":"볼트의-구성관리-저장소-및-관리","link":"#볼트의-구성관리-저장소-및-관리","children":[{"level":3,"title":"[Example 1. Spring Boot Application]","slug":"example-1-spring-boot-application","link":"#example-1-spring-boot-application","children":[]},{"level":3,"title":"[Example 2. Spring Boot Application + RBAC]","slug":"example-2-spring-boot-application-rbac","link":"#example-2-spring-boot-application-rbac","children":[]}]}],"git":{"createdTime":1681084498000,"updatedTime":1695042774000,"contributors":[{"name":"Great-Stone","email":"hahohh@gmail.com","commits":2}]},"readingTime":{"minutes":25.33,"words":1520},"filePathRelative":"04-HashiCorp/06-Vault/04-UseCase/spring-boot.md","localizedDate":"2023년 4월 10일","excerpt":"\\n<blockquote>\\n<p>Example Source : <a href=\\"https://github.com/Great-Stone/vault_springboot_example\\" target=\\"_blank\\" rel=\\"noopener noreferrer\\">https://github.com/Great-Stone/vault_springboot_example</a></p>\\n</blockquote>\\n<p>볼트는 애플리케이션(앱)의 구성관리, 특히 사용자 ID, 패스워드, Token, 인증서, 엔드포인트, AWS 자격증명 등과 같은 민감한 정보를 안전하게 저장하는 중앙 집중식 인프라를 제공한다. 서비스의 성장과 더불어, 이를 구성하는 앱은 확장과 분리 요구 사항이 발생하면 구성 관리가 어려워 진다. 특히, 시크릿 정보가 포함되는 구성 관리는 수동으로 관리하는 경우 로컬 환경을 포함한 여러 시스템에 노출되는 위험성을 갖고, 환경마다 다른 시크릿을 관리하기위한 유지 관리의 노력과 비용이 증가한다. 볼트에서 이야기하는 앱과 관련한 \\"시크릿 스프롤(퍼짐)\\" 현상은 다음과 같다.</p>","autoDesc":true}');export{y as comp,f as data};
