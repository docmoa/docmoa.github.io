import{_ as o}from"./plugin-vue_export-helper-DlAUqK2U.js";import{r as l,o as i,c as p,b as n,d as s,a as e,e as t}from"./app-DwRVeH7t.js";const c={},r=n("h1",{id:"wildfly-jboss",tabindex:"-1"},[n("a",{class:"header-anchor",href:"#wildfly-jboss"},[n("span",null,"Wildfly(Jboss)")])],-1),d={href:"https://quay.io/repository/wildfly/wildfly?tab=info",target:"_blank",rel:"noopener noreferrer"},u=n("br",null,null,-1),m={href:"https://github.com/jboss-dockerfiles/wildfly",target:"_blank",rel:"noopener noreferrer"},k=n("br",null,null,-1),v={href:"http://www.mastertheboss.com/soa-cloud/docker/deploying-applications-on-your-docker-wildfly-image/",target:"_blank",rel:"noopener noreferrer"},b=n("p",null,"Wildfly 이미지를 베이스로 기존 Dockerfile을 작성하여 빌드 후 컨테이너를 기준으로 배포하는 것도 가능하지만, 베이스 이미지를 유지한 채로 애플리케이션(war)을 바인드하여 실행하는 것도 가능하다.",-1),g=n("h2",{id:"dockerfile과-비교",tabindex:"-1"},[n("a",{class:"header-anchor",href:"#dockerfile과-비교"},[n("span",null,"Dockerfile과 비교")])],-1),y=t(`<li><p>dockerfile 의 예</p><div class="language-docker" data-ext="docker" data-title="docker"><pre class="language-docker"><code><span class="token instruction"><span class="token keyword">FROM</span> jboss/wildfly</span>
...
</code></pre></div><ul><li><code>FROM</code>은 Nomad가 실행시킬 이미지로 지정</li><li><code>RUN</code> 절의 <code>add-user.sh</code>는 <code>mgmt-users.properties</code>를 생성하고자 하는 목적이므로 Nomad의 <code>artifact</code>로 중앙레포에서 받거나 <code>template</code>으로 작성하여 바인딩 가능</li><li><code>ADD</code> 절은 WAR파일을 추가하는 것으로 호스트의 파일 또는 <code>artifact</code>로 중앙레포에서 받아 바인딩</li><li><code>CMD</code> 절은 기본 실행명령을 대체하는 것으로 Nomad에서 <code>command</code>와 <code>args</code>로 대체 가능</li></ul></li><li><p>job &gt; groups &gt; task(docker) &gt; artifact :<br> WAR 파일을 다운로드 받아 준비</p><div class="hint-container warning"><p class="hint-container-title">참고</p><p>Nomad 클라이언트 호스트에 미리 파일을 두는것도 가능하나 오케스트레이션 특성상 중랑 레포기능을 하는곳에서 배포시 다운받는 방식이 확장성 측면에서 고려되어야 함</p></div></li><li><p>job &gt; groups &gt; task(docker) &gt; template :</p><ul><li><code>add-user.sh</code>를 통해 management 콘솔의 사용자를 생성해야 하지만 미리 생성된 내용(admin/admin)을 넣어 처리</li><li>다른 사용자를 생성하고자 한다면 설치된 wildfly나 docker로 실행한 wildfly에서 다음 스크립트로 생성된 파일 내용 확인하여 template의 data로 처리<div class="language-bash" data-ext="sh" data-title="sh"><pre class="language-bash"><code><span class="token comment"># example (admin/admin)</span>
...
</code></pre></div><div class="language-bash" data-ext="sh" data-title="sh"><pre class="language-bash"><code><span class="token assign-left variable">admin</span><span class="token operator">=</span>c22052286cd5d72239a90fe193737253
</code></pre></div></li></ul></li>`,3),h=n("p",null,"job > groups > task(docker) > config > mount :",-1),q=n("ul",null,[n("li",null,[s("다운받은 WAR 파일과 "),n("code",null,"mgmt-users.properties"),s("를 컨테이너에 바인딩")]),n("li",null,"Nomad가 Host 내부적으로 별도 루트경로를 할당받으므로, 각 파일과 구성파일은 독립적으로 위치함")],-1),f={class:"hint-container tip"},w=n("p",{class:"hint-container-title"},"팁",-1),_=n("code",null,"volumes",-1),j={href:"https://www.nomadproject.io/docs/drivers/docker#volumes",target:"_blank",rel:"noopener noreferrer"},x=t(`<li><p>job &gt; groups &gt; task(docker) &gt; config &gt; args :<br> management가 기본 <code>127.0.0.1</code>이므로 포트포워딩으로 접속이 불가하므로 <code>0.0.0.0</code>으로 변경</p><div class="language-hcl" data-ext="hcl" data-title="hcl"><pre class="language-hcl"><code><span class="token property">args</span> <span class="token punctuation">=</span> <span class="token punctuation">[</span><span class="token string">&quot;/opt/jboss/wildfly/bin/standalone.sh&quot;</span>, <span class="token string">&quot;-b&quot;</span>, <span class="token string">&quot;0.0.0.0&quot;</span>, <span class="token string">&quot;-bmanagement&quot;</span>, <span class="token string">&quot;0.0.0.0&quot;</span><span class="token punctuation">]</span>
</code></pre></div><div class="hint-container tip"><p class="hint-container-title">팁</p><p>조금더 명확하게는 <code>command</code> 에 <code>&quot;/opt/jboss/wildfly/bin/standalone.sh&quot;</code>를 구성하고 args를 분리하는 것도 가능</p></div></li>`,1),N=t(`<h2 id="sample-job" tabindex="-1"><a class="header-anchor" href="#sample-job"><span>Sample Job</span></a></h2><div class="language-hcl line-numbers-mode" data-ext="hcl" data-title="hcl"><pre class="language-hcl"><code><span class="token keyword">locals</span> <span class="token punctuation">{</span>
